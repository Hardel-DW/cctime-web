...
function sanitizeCSSColor(color: string): string | null {
    ...
    if (!validColorRegex.test(color.trim())) {
        console.warn(`Invalid CSS color value: ${color}`);
        return null;
    }
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
    ...
    // Create CSS variables programmatically
            ...
            (acc, [theme, prefix]) => {
                ...
                colorConfig.forEach(([key, itemConfig]) => {
                    ...
                    if (rawColor) {
                        ...
                        if (sanitizedColor) {
                            cssVars[`--color-${key}`] = sanitizedColor;
                        }
                    }
                });

                if (Object.keys(cssVars).length > 0) {
                    acc[selector] = cssVars;
                }
    ...
    // Apply styles using CSS-in-JS approach
    React.useEffect(() => {
...
// Re-export from Recharts for backwards compatibility
export { ChartLegend, ChartTooltip } from "./chart-utils";
